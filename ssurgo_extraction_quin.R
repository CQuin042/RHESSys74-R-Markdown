### THIS IS SSURGO EXTRACTION FROM LAURENCE LIN MODIFIED
### Originally https://github.com/laurencelin/ssurgo_extraction
...
mdb.get('SSURGO/soildb_NC_2003.mdb')

ssurgo.mu <- st_read('SSURGO/soildb_NC_2003.mdb', layer = 'mapunit')
ssurgo.co <- st_read('SSURGO/soildb_NC_2003.mdb', layer = 'component')
ssurgo.hz <- st_read('SSURGO/soildb_NC_2003.mdb', layer = 'chorizon')
#new
ssurgo.tg<- st_read('SSURGO/soildb_NC_2003.mdb', layer = 'chtexturegrp')
ssurgo.tx<- st_read('SSURGO/soildb_NC_2003.mdb', layer = 'chtexture')
...
for(i in 1:length(mukey)){
	...
	componentSummaryTable = as.data.frame(t(sapply( comp[cond1,'cokey'], function(cokey){
        ...
        horizonSummaryTable$horizonThickness = (chorizon[cond2,'hzdepb_r']-chorizon[cond2,'hzdept_r'])*0.01 #cm -> m
        ...
        horizonSummaryTable$horizonWeight = horizonSummaryTable$horizonThickness / sum(horizonSummaryTable$horizonThickness, na.rm=T)
        ...
        horizonSummaryTable$kffact = as.numeric(chorizon[cond2,'kffact']) # soil erodibility factor
        ...
        return <- unlist(list(
        	id = cokey,
        	horizonThickness = sum(horizonSummaryTable$horizonThickness,na.rm=T),
        	...
        	vksat = sum(horizonSummaryTable$horizonThickness,na.rm=T) / sum(horizonSummaryTable$horizonThickness/horizonSummaryTable$ksat, na.rm=T),
	...
	componentSummaryTable<- as.data.frame(lapply(componentSummaryTable,as.numeric))
	...
	tmp = unlist(list(
		...
		soilDepth = sum(componentSummaryTable$Weight*componentSummaryTable$horizonThickness,na.rm=T),
...
write.csv(mukeyHold,'RSSsoil_mukey.csv',row.names=F)
