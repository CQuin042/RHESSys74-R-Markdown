...
```{r Run Rhessys Once for Validation}
setwd(system.file("extdata/", package = "RHESSysIOinR"))
...
plot(obsws32smvalclean$Date, 
     obsws32smvalclean$mergedsoilmoisture, 
     type = "l", 
     main = "All Site Averaged Sensor Output", 
     lty = 3, 
     lwd = 2, 
     ylim = c(0.10, 0.35), 
     xlab = "Date", 
...
lines(obsws32smvalclean$Date, obsws32smvalclean$Site1, col = 'blue', lwd = 1)
lines(obsws32smvalclean$Date, obsws32smvalclean$Site2, col = 'red', lwd = 1)
lines(obsws32smvalclean$Date, obsws32smvalclean$Site3, col = 'green', lwd = 1)

legend("topright", 
       legend = c("Averaged Output", "Site 1", "Site 2", "Site 3"), 
...
threesitevalidationplot <- ggplot(validationnona, aes(group, y = KGE, fill = site)) + 
  geom_boxplot() + 
  ylab("Soil Moisture KGE - Validation") +
  xlab("Soil Map Input") +
  ggtitle("Patch Specific Soil Moisture KGE For ALL Calibration Runs, NA Excluded") +
  geom_hline(yintercept = -0.41, linetype = 2, col = 'RED') + 
