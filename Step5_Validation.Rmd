...
{plot(cwws32_run6$bd$date,cwws32_run6$bd$streamflow, type = "l", col = 'red', main = "Observed Streamflow vs Top KGE Run", xlab = "Date", ylab = "Streamflow")
lines(obsws32$Date,obsws32$discharge_mm, col = 'orange', lwd = 2, lty = 2)}


valsubset <- simmerge6[simmerge6$Date >= Valdates[1] & simmerge6$Date <= Valdates[2], ]

ValKGE<- KGE(sim = valsubset$streamflow, obs = valsubset$observedstreamflow)


valsubsetsm<- simmergesm6[simmergesm6$Date >= Valdates[1] & simmergesm6$Date <= Valdates[2],]
