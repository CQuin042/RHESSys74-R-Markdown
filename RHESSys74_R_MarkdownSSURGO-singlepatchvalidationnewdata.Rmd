...
#lowelevationprecip<- ("C:/Users/Carlos/Desktop/ORISE/Climate Data/CW/Precipitation/coweeta-high-elevation-precipitation-2021.xlsx")
#highelevationprecip<- ("C:/Users/Carlos/Desktop/ORISE/Climate Data/CW/Precipitation/coweeta-low-elevation-precipitation-2021.xlsx")
#averagetemperature<- ("C:/Users/Carlos/Desktop/ORISE/Climate Data/CW/Precipitation/coweeta-average-air-temperature-2021.xlsx")
...
cwtcs21_selected <- cwtcs21 %>% select(date, TMINcs21=TMIN,TMAXcs21=TMAX)
cwtcs28_selected <- cwtcs28 %>% select(date, TMINcs28=TMIN, TMAXcs28=TMAX)
...
trimmed_cwtclimate <- trimmed_cwtclimate %>%
  mutate(
    RRG12 = ifelse(is.na(RRG12), coalesce(RRG20, 0), RRG12),
    RRG20 = ifelse(is.na(RRG20), coalesce(RRG12, 0), RRG20),
    TMINcs21 = ifelse(is.na(TMINcs21), TMINcs28, TMINcs21),
    TMAXcs21 = ifelse(is.na(TMAXcs21), TMAXcs28, TMAXcs21),
    TMINcs28 = ifelse(is.na(TMINcs28), TMINcs21, TMINcs28),
    TMAXcs28 = ifelse(is.na(TMAXcs28), TMAXcs21, TMAXcs28)
  )
...
### filtering process for precip data must be rechecked, rain gauges do not log on non rain days, NA should be 0 on those days but value from alternate RG is used instead, if both measure 0 or NA then 0 is assumed.

##make sure units are correct
##precip converted from inches to meters

cwtws32localrain<- rbind(trimmedstartdaterhessys, as.data.frame(trimmed_cwtclimate$RRG12 * 0.0254))
cwtws32localtmin<- rbind(trimmedstartdaterhessys, as.data.frame(trimmed_cwtclimate$TMINcs28))
cwtws32localtmax<- rbind(trimmedstartdaterhessys, as.data.frame(trimmed_cwtclimate$TMAXcs28))

cwtws32localrain<- data.frame(cwtws32localrain)
cwtws32localtmin<- data.frame(cwtws32localtmin)
cwtws32localtmax<- data.frame(cwtws32localtmax)
